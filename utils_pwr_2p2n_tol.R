# Adjust Searching Boundary for Uniroot
# tside is indicator for alternatives
...
"sam.2p2n.test" <-
            ...
            alternative = c("two.sided", "less","greater")) 
  {
    ...
    if (tside == 2)  {
      p.body <- quote({
        pnorm(qnorm(sig.level/2, lower = FALSE) - h * sqrt((n1 * n2)/(n1 + n2)), lower = FALSE) 
        + pnorm(qnorm(sig.level/2, lower = TRUE) - h * sqrt((n1 * n2)/(n1 + n2)), lower = TRUE)
      })
    }
    if (is.null(power)) 
      power <- eval(p.body)
    else if (is.null(h)){ 
      if(tside==2){       h <- uniroot(function(h) eval(p.body) - power, c(-10,10))$root} #c(1e-10,10))$root}
      if(tside==1){       h <- uniroot(function(h) eval(p.body) - power, c(-10,5))$root}
      if(tside==3){       h <- uniroot(function(h) eval(p.body) - power, c(-5,10))$root}
    }
    else if (is.null(n1)) 
      n1 <- uniroot(function(n1) eval(p.body) - power, c(2 + 1e-10, 1e+09))$root
    else if (is.null(n2)) 
      n2 <- uniroot(function(n2) eval(p.body) - power, c(2 + 1e-10, 1e+09))$root
